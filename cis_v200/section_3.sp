...
control "cis_v200_3_1" {
  ...
  query         = query.compute_network_contains_no_default_network.sql
...
control "cis_v200_3_2" {
  ...
  query         = query.compute_network_contains_no_legacy_network.sql
...
control "cis_v200_3_3" {
  ...
  query         = query.dns_managed_zone_dnssec_enabled.sql
...
control "cis_v200_3_4" {
  ...
  query         = query.dns_managed_zone_key_signing_not_using_rsasha1.sql
...
control "cis_v200_3_5" {
  ...
  description   = "DNSSEC algorithm numbers in this registry may be used in CERT RRs. Zone signing (DNSSEC) and transaction security mechanisms (SIG(0) and TSIG) make use of particular subsets of these algorithms. The algorithm used for key signing should be a recommended one and it should be strong."
  query         = query.dns_managed_zone_zone_signing_not_using_rsasha1.sql
...
control "cis_v200_3_9" {
  ...
  query         = query.compute_ssl_policy_with_no_weak_cipher.sql
...
control "cis_v200_3_10" {
  ...
  query         = query.compute_firewall_allow_connections_proxied_by_iap.sql
